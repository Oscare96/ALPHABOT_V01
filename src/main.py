...
@app.get("/")
def root(): return {"service":"alphabot-v01","version":"0.1.0","mode":"research","docs":"/docs"}

@app.get("/health")
def health(): return {"ok":True,"service":"alphabot-v01","version":"0.1.0"}
...
def backtest(start:str=Query(default="2010-01-01"),end:str|None=Query(default=None)):
    try:
        r=run_backtest(download_market_data(start=start,end=end),DEFAULT_CONFIG); e=r["equity"].reset_index(); e.columns=["date","strategy","spy"]; e["date"]=e.date.dt.date.astype(str)
        return {"strategy":"rotation-reversal-v0.1","summary":r["summary"],"equity":e.tail(500).to_dict(orient="records")}
    except Exception as e: raise HTTPException(status_code=500,detail=str(e)) from e
