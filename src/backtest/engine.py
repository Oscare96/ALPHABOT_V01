...
def run_backtest(data, config=DEFAULT_CONFIG, entry_regime=None):
    ...
    for dt in dates:
        if rebalance_mask.loc[dt]:
            ...
            holdable = day[day["symbol"].isin(held_symbols) & (day["rotation_score"] >= config.hold_score)]
            ...
            pool = pd.concat([holdable, entrants], ignore_index=True).sort_values("rotation_score", ascending=False).drop_duplicates("symbol").head(config.max_sectors)
