...
def run_backtest(data,config=DEFAULT_CONFIG,entry_regime=None,exit_policy=None,entry_score_override=None,rebalance_weeks=1,min_hold_trading_days=0,max_sectors_override=None,weighting="equal",spy_fallback=False,spy_fallback_mode="always"):
    cfg=replace(config,entry_score=float(entry_score_override)) if entry_score_override is not None else config;maxs=int(max_sectors_override or cfg.max_sectors);features=build_features(data,cfg);sector_prices=pd.DataFrame({s:data[s]["close"] for s in SECTOR_ETFS}).sort_index();spy=data[BENCHMARK]["close"].reindex(sector_prices.index).ffill();prices=sector_prices.copy();prices[BENCHMARK]=spy;returns=prices.pct_change(fill_method=None).fillna(0);spyret=returns[BENCHMARK];spy_ma200=spy.rolling(200).mean();spy_ret20=spy.pct_change(20);dates=prices.index;mask=_rebalance_mask(dates,rebalance_weeks);cols=list(SECTOR_ETFS)+([BENCHMARK] if spy_fallback else []);current=pd.Series(0.0,index=cols);wh=pd.DataFrame(0.0,index=dates,columns=cols);turn=pd.Series(0.0,index=dates);entry_loc={}
    for loc,dt in enumerate(dates):
        if mask.loc[dt]:
            ...
            if len(pool):
                ...
                allow=True
                if spy_fallback_mode=="conditional":
                    allow=bool(pd.notna(spy_ma200.loc[dt]) and spy.loc[dt]>spy_ma200.loc[dt] and pd.notna(spy_ret20.loc[dt]) and spy_ret20.loc[dt]>0)
                if allow:target.loc[BENCHMARK]=1.0
            ...
            turn.loc[dt]=float((target-current).abs().sum());current=target
        wh.loc[dt]=current
    ew=wh.shift(1).fillna(0);fallback_exposure=ew[BENCHMARK] if spy_fallback else pd.Series(0.0,index=dates);gross=(ew*returns[cols]).sum(axis=1);cost=turn*(cfg.trading_cost_bps/10000);net=gross-cost;eq=(1+net).cumprod();geq=(1+gross).cumprod();spyeq=(1+spyret).cumprod();exposure=ew.sum(axis=1);trades=_ledger(ew[list(SECTOR_ETFS.keys())],sector_prices);stats=_trade_stats(trades);summary={"start":dates.min().date().isoformat(),"end":dates.max().date().isoformat(),"strategy_total_return":float(eq.iloc[-1]-1),"gross_strategy_total_return":float(geq.iloc[-1]-1),"strategy_cagr":cagr(eq),"benchmark_total_return":float(spyeq.iloc[-1]-1),"benchmark_cagr":cagr(spyeq),"max_drawdown":max_drawdown(eq),"benchmark_max_drawdown":max_drawdown(spyeq),"sharpe":sharpe(net),"benchmark_sharpe":sharpe(spyret),"sortino":sortino(net),"annualized_volatility":annualized_volatility(net),"total_turnover":float(turn.sum()),"estimated_cost_drag_return_points":float((geq.iloc[-1]-1)-(eq.iloc[-1]-1)),"time_in_market":float((exposure>0).mean()),"average_gross_exposure":float(exposure.mean()),"spy_fallback":spy_fallback,"spy_fallback_mode":spy_fallback_mode,"fallback_time":float((fallback_exposure>0).mean()),"fallback_days":int((fallback_exposure>0).sum()),"entry_score":float(cfg.entry_score),"min_hold_trading_days":min_hold_trading_days,"max_sectors":maxs,"weighting":weighting,**stats}
